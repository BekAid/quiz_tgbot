...
async def is_admin_func(message:types.Message):
    ...
    await bot.send_message(message.from_user.id, "Выбери пожалуйста функцию из меню ;)",
...
async def cancel_handler(message:types.Message, state:FSMContext):
    if message.from_user.id == ADMIN_ID:
        ...
        await state.finish()
        await message.reply('Операция удачно отменена!')
...
async def load_photo(message: types.Message,
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        await FSADMIN.next()
        await message.reply("Админ отправь заголовок к фото")
